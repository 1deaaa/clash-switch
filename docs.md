...
6. 当前节点失败并达到阈值后，优先从本轮成功缓存中选择最低延迟候选；本轮尚无成功结果时，使用 90 秒内的上一轮成功缓存。
7. 切换后立即复核 AI Studio 网页。候选复核失败时将其从本轮可选缓存中排除，并继续尝试其它成功候选；无新鲜成功缓存时，等待本轮后续候选完成后继续处理。
8. 同一次当前节点故障只触发一次有效切换，本轮剩余探测继续完成并刷新缓存；扣除本轮耗时后等待，尽量让相邻轮次起点相隔 `interval_seconds`。
