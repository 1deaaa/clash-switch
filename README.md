...
跨 Windows 与 Linux 的 Mihomo/Clash Verge Rev 节点连通性监控器。默认每 60 秒并行测试已选候选以及当前节点，每个网址只请求一次，单次超时 5 秒。当前节点可用时保持不变；一旦失败，等本轮结果收齐后使用本轮成功结果或 90 秒内的上一轮缓存，切换到延迟最低且通过 AI Studio 网页复核的候选。
