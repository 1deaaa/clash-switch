...
跨 Windows 与 Linux 的 Mihomo/Clash Verge Rev 节点连通性监控器。默认每 60 秒并行测试已选候选以及当前节点，每个网址只请求一次，单次超时 5 秒。当前节点可用时保持不变；一旦失败，优先使用新鲜成功缓存立即切换，没有缓存时等本轮结果收齐，再切换到延迟最低且通过 AI Studio 网页复核的候选。
